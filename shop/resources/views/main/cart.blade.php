...
<div class="header-cart flex-col-l p-l-65 p-r-25">
    ...
    <div class="header-cart-content flex-w js-pscroll">
        ...
        @if(!is_null($carts))
            @php
            $sumPriceCart += $product->price_sale != 0 ?
                    $product->price_sale * $carts[$product->id] : $product->price * $carts[$product->id];
            @endphp
        @endif

        <div class="w-full">
            <div class="header-cart-total w-full p-tb-40">
                Total: @if(is_null($carts))
                            @php
                                echo 0;
                            @endphp
                        @else
                            {{ number_format($sumPriceCart, '0', '', '.') }}
                        @endif
            </div>
